...
test("map fits the width, preserves its center, and can zoom out to show its height", () => {
  for (const aspect of [16 / 9, 375 / 812]) {
    const view = resizeView(INITIAL, aspect);
    expect(view.width).toBe(INITIAL.width);
