...
Open http://localhost:3000. Tap a stop for an immediate preview, or choose **Start listening** for live music; the first snapshot plays as soon as it arrives. Drag, scroll, pinch, or use the zoom buttons to explore; Tab then Enter/Space plays a focused stop. A skip link reaches the sound controls without tabbing through the whole network.
...
- The browser asks for the next snapshot three seconds before the queued phrase runs out (`nextPollDelay` in `src/lib/arrivals.ts`; never more often than every four seconds, and every ten seconds while nothing is queued), so successive phrases chain bar to bar on one grid with no gap. Each poll replays the **entire snapshot**, including vehicles already heard on a previous poll. A tram that remains at a platform sounds again in the next phrase. Duplicate records within one snapshot are deduplicated by `(vehicleId, tripId, stopSequence)`; separate trams at the same logical stop remain separate notes. Out-of-order snapshots are ignored; an unchanged timestamp is replayable while the feed is still fresh. Hidden tabs stop polling and clear scheduled audio; returning to the tab, or refocusing the window with a suspended audio context, resumes the context and plays the current snapshot at once. If the browser refuses to resume audio outside a gesture, as iOS can, the app keeps listening, shows "Tap anywhere to resume audio." and resumes on the next tap or key press. Feed recovery silently primes the next snapshot.
...
Each stop has a **role** derived from the network itself (`src/data/network.ts`): interchanges (`hub`) are **kicks**, end-of-line loops (a hand-listed `termini` set; the diagram has no line topology) are **snares**, and every other stop is a **lead**, **hi-hat** or **bass** by a stable hash of its key, with leads the majority so phrases keep a tonal centre. Dworzec Towarowy is both an interchange and a terminus; the interchange wins. Drums are synthesised (pitch-swept sine kick, filtered-noise snare and hat); bass is the lead's filtered triangle one octave down. Drums ignore root and scale changes. Trams lay over at loops, so termini often outnumber every other role in a snapshot; snares therefore accent only the backbeat and play anywhere else as ghost notes at 30 % level (`GHOST_LEVEL`), labelled "Ghost snare" in the chip. Stop highlights and the last-note chip are tinted by role, and each stop's tooltip names it.

Each snapshot is arranged like a step sequencer across approximately 9.6 seconds on a 100 BPM eighth-note grid: four bars of 4/4 (`arrange` in `src/lib/music.ts`). Every note in a step starts at the same instant, so a kick, its bass note and a hi-hat hit together rather than one after another. Each role spreads its notes evenly over its preferred steps across the whole phrase — kicks and basses on downbeats (beats 1 and 3), snares on beats 2 and 4, hi-hats on the off-beat eighths, leads evenly over everything — and spills to the next preference only when those steps are taken, so four kicks land on the four downbeats and kick and bass share a beat rather than avoiding each other. Overflow snares go to the off-beats as ghost notes and take a downbeat only when even those are full. Capacity is per role (`STEP_CAPACITY`): a step holds at most one kick, snare, hi-hat and bass, since a doubled drum hit is only a louder hit, and up to three leads, which stack into a chord. Every valid vehicle record in the phrase plays once; order within a role follows the feed. A snapshot with more notes of one role than its steps can hold extends the phrase by whole bars rather than dropping notes; subsequent phrases queue behind it, each starting on the step after the previous phrase's last bar. The sixteen-voice limit includes manual notes and future reservations, and each voice holds its slot only for its own length (`ROLE_LENGTH`: a hi-hat 50 ms, a lead 240 ms), so a full step of seven voices never crowds out manual previews. Extreme manual bursts wait for a voice rather than clipping or cutting an existing note. This is a musical replay of **trams currently stopped**, not an instant one-off arrival alert; a note may play roughly ten seconds after its snapshot was received.
...
Automated tests cover the CSV importer, platform ownership, role assignment, drum and bass voices, step-sequencer arrangement by role with simultaneous steps and ghost snares, per-role voice reservations, whole-snapshot replay, phrase chaining and poll timing, within-snapshot deduplication, silent priming, invalid feeds, real timeout expiry, HTTP failure/recovery, scale pitches and scheduling limits. Lint, TypeScript and the production build pass.
